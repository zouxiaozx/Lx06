...
namespace DAL
{
    class Comment
    {
        ...
        public IEnumerable<Model.Comment> GetAll()
        {
            using(IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "select * from Commenet";
        ...
        public IEnumerable<Model.Comment> GetWorkPage (Model.CommentPage page)
        {
            using (IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "with a as(select row_number() over(order by commentTime desc)as num,Comment.* form Comment where Comment.workId=@workId)";
                sql += "select *from a where num between(@pageIndex-1)*@pageSize+1 and @pageIndex*@pageSize;";
                return cn.Query<Model.CommentNo>(sql, page);
            }
        }
        public int Add(Model.Comment comment)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "insert into Comment(commentID,workId,userName,CommentContent,CommentTime)" + "value(@commentID,@workId,@userName,@CommentContent,@CommentTime);";
                sql += "SELECT @@IDENTITY";
